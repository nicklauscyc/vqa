...
def dummy_attention(input, attention):
    #return a uniform map
    #return 2d tensor where each entry is of size g*v and has value of 1/g*v -> normalized equal weights
    n, v = input.size()[:2]
    g = attention.size(1)
    w = 1/(g*v)
    vec = np.full((n,g*v),w)
    t_vec = torch.from_numpy(vec).type('torch.FloatTensor')
    return t_vec
